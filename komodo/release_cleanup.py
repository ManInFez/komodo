...
def run_cleanup(args, parser):
    if not args.check and not args.output:
        parser.error(
            ArgumentError(
                message="Either check flag must be set or output file must be given!",
    ...
    unused_versions = find_unused_versions(used_versions, args.repository)
    ...
    if args.output:
        remove_unused_versions(
            repository=args.repository, unused_versions=unused_versions
        )
        output_file = args.output
        write_to_file(args.repository, output_file)
        if unused_versions: 
